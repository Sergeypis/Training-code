...
class Rectangle:
    ...
    def validation_init(self, corner1: Union[float, int], corner2: Union[float, int]) -> None:
        """
        ...
        if (not isinstance(corner1[0], (float, int))) or (not isinstance(corner1[1], (float, int))):
            raise TypeError(f"Не верный тип данных 'corner1' - {type(corner1)}")
        if (not isinstance(corner2[0], (float, int))) or (not isinstance(corner2[1], (float, int))):
